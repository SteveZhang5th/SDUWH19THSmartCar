...
bool serial_init(void) {
    ...
    if (_serial_port == nullptr) {
        std::cerr << "serial: critical: Serial create failed." << std::endl;
        return true;
    }
    try {
        ...
        std::cerr << "serial: critical: Serial port open failed..." << std::endl;
        return true;
    } catch (const LibSerial::AlreadyOpen &) {
        std::cerr << "serial: critical: Serial port already open..." << std::endl;
        return true;
    } catch (...) {
        std::cerr << "serial: critical: Serial port exception..." << std::endl;
        return true;
    }
    return false;
...
bool serial_send_blocking(uint8_t c) {
    ...
    try {
        ...
        std::cerr << "serial: critical: The runtime_error." << std::endl;
        return true;
    } catch (const LibSerial::NotOpen &) {
        std::cerr << "serial: critical: Port Not Open..." << std::endl;
        return true;
    } catch (...) {
        std::cerr << "serial: critical: Serial port exception..." << std::endl;
        return true;
...
bool serial_recv_poll(uint8_t *c) {
    ...
    try {
        ...
        std::cerr << "serial: critical: The ReadByte() call has timed out." << std::endl;
        return true;
    } catch (const LibSerial::NotOpen &) {
        std::cerr << "serial: critical: Port Not Open ..." << std::endl;
        return true;
