...
std::vector<PredictResult> ai_results; //AI推理结果
...
public:
    ...
    void render()
    {
        ...
        for (int i = 0; i < prod; i += 6)
        {
            ...
            if (result.score < 0.50)   //阈值
            {
                continue;
            }
            // else if(result.type == 3 && result.score < 0.30){
            //     continue;
            // }
    ...
    {
        for(int i=0;i<ai_results.size();i++)
        {
            PredictResult result = ai_results[i];
            ...
            cv::putText(img, label_name, cv::Point(result.x, result.y), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 254), 1);
        }
    }
...
//颜色分离
cv::Mat Color_Split(cv::Mat imgOriginal)
{
	cv::Mat imgHSV;
	// GaussianBlur(imgOriginal, imgOriginal, cv::Size(7, 7), 0, 0);
	...
	cv::equalizeHist(hsvSplit[2], hsvSplit[2]);    //对HSV的亮度通道进行直方图均衡
	merge(hsvSplit, imgHSV);				   //合并三种通道
	cv::Mat ce1;
	cv::Mat ce2;
	cv::Mat ce3, ce4;
	inRange(imgHSV, cv::Scalar(156, 43, 46), cv::Scalar(180, 255, 255), ce1); //红色
	inRange(imgHSV, cv::Scalar(0, 43, 46), cv::Scalar(10, 255, 255), ce2); //红色
    inRange(imgHSV, cv::Scalar(26, 43, 46), cv::Scalar(50, 255, 255), ce3);//黄色
	ce4 = ce1 + ce2 + ce3;
    return ce4;
}
