...
int main(int argc, char *argv[]) {
  ...
  // ret = comm_init();
  // assert(!ret);
  if(UartSendEnable && !replay_enabled){
       ...
       static int speed_real = 0, mileage_real = 0; // 单位 mm/s mm
        ...
        if (meta_enabled) {
            if (record_enabled) {
                // 保存状态到文件中
                static char buf[100];
                int len = sprintf(buf, "%d %d %d", (int)do_timer, mileage_real, speed_real);
                assert(len >= 0);
                assert(len <= 100);
                ...
                int tmp[10];
                ret = sscanf(buf, "%d%d%d", &tmp[0], &mileage_real, &speed_real);
                assert(len >= 0);
                do_timer = tmp[0];
            }
        }
        ...
        if(UartSendEnable && !replay_enabled){
          ...
          if(Emergencyflag)
            usb_uart_send_4_0(fd, sendmessage, 0, -controldata, 0, 0);//
          else
            usb_uart_send_4_0(fd, sendmessage, motordata, -controldata, 0, 0);//
        }
        ...
        if(EmergencyStopflag){
          ...
          if(elementflag == InMainten || elementflag == OutMainten || BlackCount < 200)
            Stopt = 0;

          if(dofhead < 3 && CountOutMainten < 5){
            Stopt++;
            if(Stopt >= 15){
        ...
        if(visionenable && replay_enabled){
          Vision();
